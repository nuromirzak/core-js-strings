...
function removeFirstOccurrences(str, value) {
  const removeFirstOccurrenceIndex = str.indexOf(value);
  if (removeFirstOccurrenceIndex === -1) return str;
  return (
    str.slice(0, removeFirstOccurrenceIndex) +
    str.slice(removeFirstOccurrenceIndex + value.length)
  );
}
...
function removeLastOccurrences(str, value) {
  const removeLastOccurrenceIndex = str.lastIndexOf(value);
  if (removeLastOccurrenceIndex === -1) return str;
  return (
    str.slice(0, removeLastOccurrenceIndex) +
    str.slice(removeLastOccurrenceIndex + value.length)
  );
}
...
function sumOfCodes(str) {
  return str ? [...str].reduce((acc, char) => acc + char.charCodeAt(0), 0) : 0;
}
...
function getStringFromTemplate(firstName, lastName) {
  const myTag = (strings, str1, str2) => {
    return `${strings[0]}${str1}${strings[1]}${str2}${strings[2]}`;
  };
  return myTag`Hello, ${firstName} ${lastName}!`;
}
...
function unbracketTag(str) {
  ...
  return (
    str.slice(0, firstBracketIndex) +
    str.slice(firstBracketIndex + 1, lastBracketIndex) +
    str.slice(lastBracketIndex + 1)
  );
}
...
function encodeToRot13(str) {
  ...
  const encodedChars = [...str].map((char) => {
    const foundIndex = alphabet.indexOf(char);
    if (foundIndex === -1) return char;
    return rot13Alphabet[foundIndex];
  });
...
function getCardId(value) {
  ...
  const row = suits.indexOf(value[value.length - 1]);
  const col = ranks.indexOf(value[0]);
